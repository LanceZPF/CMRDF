...
def calc_loss(view1_feature, view2_feature, view1_predict, view2_predict, labels_1, labels_2, alpha, beta):
    ...
    term11 = torch.sum(torch.log(1+torch.exp(theta11)).float() - Sim11 * theta11)
    term12 = torch.sum(torch.log(1+torch.exp(theta12)) - Sim12 * theta12)
    term13 = torch.sum(torch.log(1 + torch.exp(theta22)) - Sim22 * theta22)
    term1 = term11 + term12 + term13
    term2 = torch.sqrt(torch.sum((view1_feature-view2_feature)**2)) / 2.
    term3 = (torch.sqrt(torch.sum((view1_predict-labels_1.float())**2)) +
             torch.sqrt(torch.sum((view2_predict-labels_2.float())**2))).float() / 2.

    batch_size = view1_feature.shape[0]
    im_loss = term3/batch_size + alpha * term1 / (batch_size**2) + beta * term2/batch_size
    return im_loss
